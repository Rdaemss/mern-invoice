...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  if (!passwordConfirm) {
    res.status(400);
    throw new Error("A confirmed password field is required");
  }
  ...
  if (user && passwordResetToken) {
    ...
    res.json({
      success: true,
      message: `Hey ${username.firstName}, Your password reset was successful. An email has been sent to confirm the same`,
    });
  }
});
